...
final class PlayQueue {
    enum Mode: Int { case sequential = 0, repeatOne = 1, shuffle = 2 }
    ...
    func cycleMode() { mode = Mode(rawValue: (mode.rawValue + 1) % 3) ?? .sequential }
    var modeLabel: String {
        switch mode { case .repeatOne: return "单曲"; case .shuffle: return "随机"; default: return "顺序" }
    }
    func label() -> String {
